...
class DinoUnit:
    def __init__(self):
        self.dino_model_cache = OrderedDict()
        self.sam_extension_dir = scripts.basedir()
        self.dino_model_dir = os.path.join(self.sam_extension_dir, "models/grounding-dino")
        ...
        self.dino_install_issue_text = ("permanently switch to local groundingdino on Settings/Segment Anything "
                                        "or submit an issue to https://github.com/IDEA-Research/Grounded-Segment-Anything/issues.")


    def install_goundingdino(self):
    ...
    def show_boxes(self, image_np, boxes, color=(255, 0, 0, 255), thickness=2, show_index=False):
        if boxes is None:
            return image_np
    ...
    def clear_dino_cache(self):
        self.dino_model_cache.clear()
        gc.collect()
        torch_gc()


    def load_dino_model(self, dino_checkpoint, dino_install_success):
        print(f"Initializing GroundingDINO {dino_checkpoint}")
        if dino_checkpoint in self.dino_model_cache:
            dino = self.dino_model_cache[dino_checkpoint]
            if shared.cmd_opts.lowvram:
                dino.to(device=device)
        else:
            self.clear_dino_cache()
            ...
            dino.to(device=device)
            self.dino_model_cache[dino_checkpoint] = dino
        dino.eval()
        return dino


    def load_dino_image(self, image_pil, dino_install_success):
        if dino_install_success:
    ...
    def dino_predict_internal(self, input_image, dino_model_name, text_prompt, box_threshold):
        install_success = self.install_goundingdino()
        print("Running GroundingDINO Inference")
        dino_image = self.load_dino_image(input_image.convert("RGB"), install_success)
        dino_model = self.load_dino_model(dino_model_name, install_success)
